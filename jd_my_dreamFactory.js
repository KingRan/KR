...
const notHelpList = ['jd_4ec5358c7990f', 'jd_5b70fb199971f', 'jd_73650adb20cc8', '%E9%9A%8F%E9%A3%8E%E8%BF%9C%E7%A8%8B']
const xxx = ['121388038', 'jd_70892f8a2c567']
jdCookieNode = Object.values(jdCookieNode).filter((item) => xxx.filter((item2) => item.indexOf(item2) >= 0).length <= 0)
jdCookieNode = Object.fromEntries(jdCookieNode.map((item, index) => ([`CookieJD${index > 0 ? index + 1 : ''}`, item])))
...
!(async () => {
  // cookiesArr = Array.from(new Set([cookiesArr[4], ...cookiesArr]))
...
async function helpFriends() {
  ...
  if ($.canHelpFlag || $._canHelpNum > 0) {
    ...
    if(new Date().getHours() < 16){
      const _newShareCode = [...$.newShareCode, ...$.newShareCode, ...$.newShareCode, ...$.newShareCode]
      const maxHelpNum = Math.floor(cookiesArr.length*3/$.newShareCode.length)
      const multiple = (Math.ceil($.index/maxHelpNum) - 1)*3
      $.newShareCode = _newShareCode.splice(multiple, 3)
    }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              $._canHelpNum = $._canHelpNum + assistNumMax - assistListToday.length
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
                ...
                // 可以兑换商品、商品兑换已超时、商品生产已失效都不加入助力
                if(!notHelpList.includes($.UserName) && production.status !== 3 && data.productionList && ![1, 3].includes(production['exchangeStatus'])) {
