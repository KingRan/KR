...
*/
const $ = new Env('东东农场');
let cookiesArr = [], cookie = '', jdFruitShareArr = [], isBox = false, notify, newShareCodes, allMessage = '', jdFruitShareFullArr = [];
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
        $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.UserName || $.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, { "open-url": "https://bean.m.jd.com/bean/signIndex.action" });
  ...
  if ($.isNode() && allMessage && $.ctrTemp) {
    ...
    // const _allMessageLen = allMessage.match(/【京东账号.*?\】/gi)
    // await notify.sendNotify(`${$.name}`, `${allMessage.substring(0, allMessage.indexOf(_allMessageLen[Math.min(30,_allMessageLen.length-1)]))}`)
    // // await $.wait(1000)
    // // await notify.sendNotify(`${$.name}`, `${allMessage.substring(allMessage.indexOf(_allMessageLen[19]), allMessage.indexOf(_allMessageLen[Math.min(39,_allMessageLen.length-1)]))}`)
    // if(_allMessageLen.length > 30) {
    //   await $.wait(1000)
    //   await notify.sendNotify(`${$.name}`, `${allMessage.substring(allMessage.indexOf(_allMessageLen[Math.min(30)]))}`)
    // }
  }
})()
  .catch((e) => {
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        ...
        $.msg($.name, ``, `【京东账号${$.index}】${$.UserName || $.UserName}\n【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n请去京东APP或微信小程序查看\n点击弹窗即达`, option);
        if ($.isNode() && !process.env.jdFruitNotNotifyList.includes($.UserName)) {
          await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.UserName}水果已可领取`, `【京东账号${$.index}】${$.UserName || $.UserName}\n【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n请去京东APP或微信小程序查看`);
        }
        return
    ...
    $.logErr(e);
    // const errMsg = `京东账号${$.index} ${$.UserName || $.UserName}\n任务执行异常，请检查执行日志 ‼️‼️`;
...
async function doTenWater() {
  ...
  if ($.farmTask.totalWaterTaskInit.totalWaterTaskTimes < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    ...
    if (isFruitFinished) {
      ...
      $.done();
      if ($.isNode() && !process.env.jdFruitNotNotifyList.includes($.UserName)) {
        await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.UserName || $.UserName}水果已可领取`, `京东账号${$.index} ${$.UserName}\n${$.farmInfo.farmUserPro.name}已可领取`);
      }
    }
  } else {
    console.log('\n今日已完成10次浇水任务\n');
  ...
  if ($.farmAssistResult.code === "0") {
    if ($.farmAssistResult.assistFriendList && $.farmAssistResult.assistFriendList.length >= 2) {
      if ($.farmAssistResult.status === 2) {
        ...
        jdFruitShareFullArr.push($.farmInfo.farmUserPro.shareCode)
    ...
    if ($.masterHelpResult.code === '0') {
      ...
      console.log('领取额外奖励水滴结束\n');
      message += `【助力您的好友】${($.masterHelpResult.masterHelpPeoples || []).length}\n`;
    }
  }
}
//助力好友
async function masterHelpShare() {
  ...
  const _newShareCodes = newShareCodes.filter((item) => !jdFruitShareFullArr.includes(item))
  for (let code of _newShareCodes) {
...
async function receiveFriendInvite() {
  let friendShareCodes = [...newShareCodes]
  const readShareCodeRes = await readShareCode();
  if (readShareCodeRes && readShareCodeRes.code === 200) {
    friendShareCodes = [...new Set([...friendShareCodes, ...(readShareCodeRes.data || [])])];
  }
  for (let code of friendShareCodes) {
    if (code === $.farmInfo.farmUserPro.shareCode) {
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    newShareCodes = [...jdFruitShareArr].filter(Boolean)
        ...
        if (err) {
          ...
          if (data) {
            ...
            if (data['retcode'] === "0" && data.data && data.data.hasOwnProperty("userInfo")) {
              // $.UserName = data.data.userInfo.baseInfo.nickname;
            }
          } else {
            $.log('京东服务器返回空数据');
          }
