...
$.newShareCodes = []
const inviteCodes = ['T012v_9yQxYR9VffCjVfnoaW5kRrbA']
  ...
  return new Promise(async resolve => {
    ...
    $.post(options, async (err, resp, data) => {
      try {
        if (safeGet(data)) {
          data = $.toObj(data)
          let beans = data.data.result.jBeans.filter((item) => item.status != 1).sort((a, b) => (b.title - a.title)).filter(x => x.status !== 1)
          if (beans.length !== 0) {
            for (let key of Object.keys(data.data.result.jBeans)) {
              await $.wait(1000)
              if(await exchange(beans[key].type, beans[key].id) === 'success') break
            }
          } else {
