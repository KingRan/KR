...
!(async () => {
  // const xxx = ['121388038', 'qin%E7%90%B40823', 'jd_fMOTUvgIPZGX', 'jd_70892f8a2c567', 'jd_OzopTeZYkdKe']
  // const _cookies = cookiesArr.filter((item) => xxx.includes(item.split(';').filter(Boolean)[1].split('=')[1])).sort(() => Math.random() - 0.5)
  // cookiesArr = Array.from(new Set([..._cookies, ...cookiesArr]))
  ...
  console.log('\n##################开始账号内互助(红包)#################\n');
  console.log('获取到的数量：', $.inviteCodeList_hb.length)
  $.inviteCodeList_hb = [...$.inviteCodeList_hb].filter((item) => item)
  $.inviteCodeList_hb.length = Math.min($.inviteCodeList_hb.length, 10)
  console.log('红包助力：', $.inviteCodeList_hb)
  if(new Date().getHours() > 14) {
    ...
    for(let i = 0;i<cookiesArr.length;i++){
      ...
      token = await getJxToken()
      $.UserName = decodeURIComponent($.cookie.match(/pt_pin=(.+?);/) && $.cookie.match(/pt_pin=(.+?);/)[1])
      for (let j = 0; j < $.inviteCodeList_hb.length && $.canHelp; j++) {
        $.code = $.inviteCodeList_hb[j]
        console.log(`\n${$.UserName}去助力：${$.code}\n`);
        await takeGetRequest('help_hb');
        await $.wait(Math.floor(Math.random()*(6-3+1)+3) * 1000);
      }
    }
  }
...
async function pasture() {
  try {
    ...
    if (Number($.homeInfo.coins) > 5000) {
      ...
      for (let j = 0; j < Math.min(canBuyTimes, 50); j++) {
...
async function takeGetRequest(type) {
  ...
  switch (type) {
    ...
    case 'GetInviteStatus':
      url = `https://m.jingxi.com/jxmc/operservice/GetInviteStatus?channel=7&sceneid=1001&activeid=${$.activeid}&activekey=${$.activekey}&jxmc_jstoken=${token['farm_jstoken']}&timestamp=${token['timestamp']}&phoneid=${token['phoneid']}&_stk=channel%2Csceneid%2Ctype&_ste=1`;
      url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
    ...
    case 'help_hb':
      url = `https://m.jingxi.com/jxmc/operservice/InviteEnroll?sharekey=${$.code}&channel=7&sceneid=1001&activeid=${$.activeid}&activekey=${$.activekey}&jxmc_jstoken=${token['farm_jstoken']}&timestamp=${token['timestamp']}&phoneid=${token['phoneid']}&_stk=activeid%2Cactivekey%2Cchannel%2Cjxmc_jstoken%2Cphoneid%2Csceneid%2Csharekey%2Ctimestamp&_ste=1`;
      url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
    ...
    case 'GetInviteStatus':
      ...
      if (data.ret === 0) {
        if(data.data.sharekey) {
          $.inviteCodeList_hb.push(data.data.sharekey)
          console.log('红包互助码：', data.data.sharekey)
        }
      }
      break;
    case 'DrawCard':
