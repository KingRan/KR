        ...
        if (err) {
          ...
          if (data.iRet === 0) {
            ...
            if (data.cardInfo.dwWorkingType === 0) {
              $.canuse = false;
              let vo = MaxFn(23-new Date().getHours(), data.cardInfo.coincard, 'dwCardNums')
              if(vo) {
                $.canuse = true;
                for (let i=0; i<Math.min(vo.dwCardNums, (23-new Date().getHours())); i++) {
                  await $.wait(3000)
...
function MaxFn (num, arr, key) {
  const list = arr.filter((item) => item[key] > 0).sort((a, b) => a[key] - b[key])
  let result = list[0];
  for (let i = 1, len = list.length; i < len; i++) {
    if (Math.abs(list[i][key] - num) < Math.abs(result[key] - num)) {
      result = list[i];
    };
  };
  return result;
      ...
      try {
        if (err) {
          ...
          if (type) {
            ...
            console.log(`【${buildNmae}】升级需要${data.ddwNextLvlCostCoin}金币，保留升级需要的8倍${data.ddwNextLvlCostCoin * 8}金币，当前拥有${$.info.ddwCoinBalance}金币`)
            if(data.dwCanLvlUp > 0 && $.info.ddwCoinBalance >= (data.ddwNextLvlCostCoin * 8)) {
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    $.newShareCodes.length = Math.min(10, $.newShareCodes.length)
